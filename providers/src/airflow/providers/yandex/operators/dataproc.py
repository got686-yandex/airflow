...
class DataprocStopClusterOperator(DataprocBaseOperator):
    ...
    def __init__(self, *, yandex_conn_id: str | None = None, decommission_timeout: int = 0, cluster_id: str | None = None, **kwargs) -> None:
        super().__init__(yandex_conn_id=yandex_conn_id, cluster_id=cluster_id, **kwargs)
        self.decommission_timeout = decommission_timeout

    def execute(self, context: Context):
        hook = self._setup(context)
        hook.dataproc_client.stop_cluster(cluster_id=self.cluster_id, decommission_timeout=self.decommission_timeout)
